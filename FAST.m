...
% starmap_filt2=zeros(size(starmap_filt));
%
% for i=1:size(Spoints,1)
%     starmap_filt2(round(Spoints(i,2)),round(Spoints(i,1)),round(Spoints(i,3)))=100;
% end
% starmap_filt2=imgaussfilt3(starmap_filt2,5);
%
%
...
%
%
% starmap_filt2=starmap_filt2.*starmap_filt;
...
% for i=1:size(Spoints,1)
%     newpatchradius=patch_bound(Spoints(i,[2 1 3]),patchradius(1:3),size(starmap_filt));
%     S_patch{i}=max(starmap_filt(Spoints(i,2)-newpatchradius(1):Spoints(i,2)+newpatchradius(1),Spoints(i,1)-newpatchradius(2):Spoints(i,1)+newpatchradius(2),Spoints(i,3)-newpatchradius(3):Spoints(i,3)+newpatchradius(3)),[],3);
...
for t=1:length(R)
    ...
    A=max(starmap_moved(:,:,max(Zpoints(z_idx(t),3)-5,1):min(Zpoints(z_idx(t),3)+5,size(starmap_moved,3))),[],3);
    B=max(minmax(log1p(Zstack_filt(:,:,max(Zpoints(z_idx(t),3)-5,1):min(Zpoints(z_idx(t),3)+5,size(starmap_moved,3))))),[],3);
    ...
    Ap=max(starmap_moved(Zpoints(z_idx(1),2)-newpatchradius(2):Zpoints(z_idx(1),2)+newpatchradius(2),Zpoints(z_idx(1),1)-newpatchradius(1):Zpoints(z_idx(1),1)+newpatchradius(1),max(Zpoints(z_idx(t),3)-5,1):min(Zpoints(z_idx(t),3)+5,size(starmap_moved,3))),[],3);
    Bp=max(minmax(log1p(Zstack_filt(Zpoints(z_idx(1),2)-newpatchradius(2):Zpoints(z_idx(1),2)+newpatchradius(2),Zpoints(z_idx(1),1)-newpatchradius(1):Zpoints(z_idx(1),1)+newpatchradius(1),max(Zpoints(z_idx(t),3)-5,1):min(Zpoints(z_idx(t),3)+5,size(starmap_moved,3))))),[],3);
